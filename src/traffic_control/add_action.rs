...
use netlink_packet_core::{NetlinkMessage, NLM_F_ACK, NLM_F_EXCL, NLM_F_REPLACE, NLM_F_REQUEST};
use netlink_packet_route::RouteNetlinkMessage;
use netlink_packet_route::tc::{TcAction, TcActionAttribute, TcActionMessage, TcActionMessageNla, TcAttribute, TcFilterFlower, TcFilterFlowerOption, TcHandle, TcHeader, TcMessage, TcOption};
use nix::libc::RTM_NEWACTION;
use crate::{Error, Handle, try_nl, try_rtnl};
...
impl TrafficActionNewRequest {
    ...
    pub fn action(mut self, action: TcAction) -> Self {
        self.message.attributes.push(
            TcActionMessageNla::Actions(vec![action])
        );
        self
    }

    /// Execute the request
    pub async fn execute(self) -> impl TryStream<Ok = TcActionMessage, Error = Error>{
        let Self {
            mut handle,
            message,
        } = self;
