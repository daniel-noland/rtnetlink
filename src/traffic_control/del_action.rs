...
use netlink_packet_route::tc::{TcAction, TcActionMessage, TcActionMessageNla};
...
impl TrafficActionDelRequest {
    pub(crate) fn new(handle: Handle) -> Self {
        TrafficActionDelRequest { handle, message: TcActionMessage::default() }
    }

    pub fn action(mut self, action: TcAction) -> Self {
        self.message.attributes.push(TcActionMessageNla::Actions(vec![action]));
        self
    }
